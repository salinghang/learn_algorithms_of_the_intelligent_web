 ...
 * @Description: 排名的搜索类
 * @author anduo
 * @date 2015年1月29日 下午2:42:08
 *
 */
public abstract class Rank {

  /**
   * This is the percentage of time that a random surfer follows the structure of the web.
   */
  ...
  public void findPageRank(double alpha, double epsilon) {
     ...
     * 
     * The following code defines the contribution of the dangling nodes, i.e. jumping randomly on a
     * page that is not connected with the one that our surfer is currently viewing
     * 
     * Notice that it is the same for all pages. An interesting variation of the algorithm would
     * introduce a "teleportation" contribution that relates the probability of an arbitrary
     * transition to the degree of interest that a user has for the content of a page.
     * 
     * Exercise: Could that be done? If so, how? What problems can you see with that variation?
     */
   ...
   * 
   * The following code defines the contribution of the dangling nodes, i.e. nodes that do not link
   * to any other node.
   * 
   * Notice that the 1/n contribution is arbitrary. Given that we have no other information about
   * the random surfer's habits or preferences, the 1/n value is fair. However, an interesting
   * variation would take into account some statistics related to the number of visits a page gets.
   * 
   * Exercise: Change the algorithm, so that a dangling node's contribution depends on some page
   * visit statistic. You can practice with a small set of pages and examine the effect on the
