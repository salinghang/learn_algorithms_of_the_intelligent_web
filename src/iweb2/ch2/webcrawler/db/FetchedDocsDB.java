...
public class FetchedDocsDB {
  ...
  private void init(boolean keepExistingData) {
    ...
    if (rootDirFile.exists()) {
      if (keepExistingData) {
        ...
        File[] existingFileGroups = rootDirFile.listFiles(new FileFilter() {
  ...
  public FetchedDocument getDocument(String documentId) {
    ...
    if (!propsFile.exists()) {
      throw new RuntimeException("Properties for document with id: '" + documentId
          + "' don't exist.");
    }
  ...
  private void writeProperty(BufferedWriter w, String key, String value) throws IOException {
    w.write(key);
    w.write(":");
    if (value != null) {
      w.write(value);
