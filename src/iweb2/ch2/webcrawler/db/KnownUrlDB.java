...
public class KnownUrlDB {
  ...
  private void writeRecord(BufferedWriter w, KnownUrlEntry ku) throws IOException {

    w.write(ku.getStatus() + FIELD_DELIMITER + String.valueOf(ku.getDepth()) + FIELD_DELIMITER
        + ku.getUrl());
    w.newLine();
  ...
  private void load() {
    try {
      ...
      while ((line = br.readLine()) != null) {
        ...
        int secondDelimiterIndex = line.indexOf(FIELD_DELIMITER, delimiterIndex + 1);
        int depth =
            Integer.valueOf(line.substring(delimiterIndex + FIELD_DELIMITER.length(),
                secondDelimiterIndex));
        String url = line.substring(secondDelimiterIndex + FIELD_DELIMITER.length());
  ...
  public boolean isSuccessfullyProcessed(String url) {
    ...
    if (r != null && KnownUrlEntry.STATUS_PROCESSED_SUCCESS.equalsIgnoreCase(r.getStatus())) {
      return true;
    } else {
      return false;
    }
  ...
  public boolean isKnownUrl(String url) {
    return processedURLs.containsKey(url) || unprocessedURLs.containsKey(url);
  }
  ...
  /**
   * 
   * @Title: findUnprocessedUrls
   * @Description: 找到没有处理的url列表
   * @param maxDocs
   * @param depth
   * @return List<String>
   */
