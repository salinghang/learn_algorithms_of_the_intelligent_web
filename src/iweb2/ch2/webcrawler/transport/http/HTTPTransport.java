 ...
 * @Description: 用来从http请求中提取数据文本，使用了HttpClient
 * @author anduo
 * @date 2015年1月29日 下午2:27:31
 *
 */
public class HTTPTransport implements Transport {
  ...
  public void init() {
    ...
    httpclient.getParams().setParameter(HttpClientParams.ALLOW_CIRCULAR_REDIRECTS, Boolean.TRUE);
    // Set default number of connections per host to 1
    httpclient.getHttpConnectionManager().getParams()
        .setMaxConnectionsPerHost(HostConfiguration.ANY_HOST_CONFIGURATION, 1);
  ...
  public FetchedDocument fetch(String documentUrl) throws TransportException {
    ...
    try {
      ...
      if (result != 200) {
        String errorMessage =
            "HTTP Response code: " + result + ", status text: " + httpget.getStatusText()
                + " for url: '" + documentUrl + "'.";
  ...
  private FetchedDocument createDocument(String targetURL, GetMethod httpget) throws IOException,
    ...
    /*
     * Check if server sent content in compressed form and uncompress the content if necessary.
     */
     ...
     * 
     * It is easy if we have Content-Type http header. In cases when this header is missing or for
     * protocols that don't pass metadata about the documents (ftp://, file://) we would have to
     * resort to url and/or content analysis to determine MIME type.
     */
    ...
    /*
     * Determine Character encoding used in the document. In some cases it may be specified in the
     * http header, in html file itself or we have to perform content analysis to choose the
     * encoding.
     */
