...
/**
 * 
 * @ClassName: BasicWebCrawler
 * @Description: 简单的网页爬虫
 * @author anduo
 * @date 2015年1月29日 下午2:14:52
 *
 */
public class BasicWebCrawler {
  ...
  /**
   * 
   * @Title: fetchAndProcess
   * @Description: 抓取网页，并且处理数据文件
   * @param maxDepth 爬取深度
   * @param maxDocs 爬取的最大文档数
   */
  public void fetchAndProcess(int maxDepth, int maxDocs) {

    boolean maxUrlsLimitReached = false; // 标识是否达到最大的url限制
    int documentGroup = 1;
    ...
    for (int depth = 0; depth < maxDepth; depth++) {
      ...
      boolean noMoreUrlsAtThisDepth = false;// 标识当前深度没有可以进行抓取的url

      while (maxUrlsLimitReached == false && noMoreUrlsAtThisDepth == false) {

        System.out.println("Starting url group: " + documentGroup + ", current depth: " + depth
            + ", total known urls: " + crawlData.getKnownUrlsDB().getTotalUrlCount()
            + ", maxDepth: " + maxDepth + ", maxDocs: " + maxDocs + ", maxDocs per group: "
            + maxBatchSize + ", pause between docs: " + pauseBetweenFetchesInMillis + "(ms)");


        // 找到当前深度下的未被处理的url，限制条件为批量最大显示数据
        ...
        System.out.println("Finished url group: " + documentGroup
            + ", urls processed in this group: " + lastProcessedBatchSize + ", current depth: "
            + depth + ", total urls processed: " + processedUrlCount);
   ...
   * 
   * @Title: fetchPages
   * @Description: 抓取网页数据放到FetchedDocsDB中
   * @param urls
   * @param fetchedDocsDB
   * @param groupId
   */
  ...
  private void processPages(String groupId, ProcessedDocsDB parsedDocsService,
    ...
    for (String id : docIds) {
      ...
      try {
        ...
        DocumentParser docParser =
            DocumentParserFactory.getInstance().getDocumentParser(doc.getContentType());
